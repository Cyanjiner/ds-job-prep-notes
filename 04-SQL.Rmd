...
/* use other aggregations in windowing function */
SELECT .....
       COUNT(...)
...
### **OTHER Common used window functions**
...
-   Getting the next value with `LEAD()` and previous value with `LAG()`
    ...
    -   provides the ability to query the value from NEXT or PREVIOUSrow
...
## COMMON ASKED PRACTICES

### Select rows with even or odd values

-   In **PostgreSQL, My SQL, Oracle, we can use `MOD()`** to check remainder.

    ```{sql }
    /* find column w/ even values */
    SELECT *
    FROM table_name
    WHERE mod(column_name, 2) = 0;
    -- if finding column w/ odd values then
    WHERE mod(column_name, 2) <> 0;
    ```

-   In **MS SQL Server, there is no MOD so we use `%`** instead.

    ```{sql}
    -- replace conditions in WHERE clause as above with following:
    WHERE column_name % 2 = 0; -- for even number
    WHERE column_name % 2 <> 0; -- for odd number
    ```

### Query the name of any student in STUDENTS who score higher than 75 marks + order output by LAST 3 chars of each name. If 2 or more students both
